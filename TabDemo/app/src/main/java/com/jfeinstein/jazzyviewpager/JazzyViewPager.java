...
public class JazzyViewPager extends ViewPager {
	...
	public JazzyViewPager(Context context, AttributeSet attrs) {
		...
		//Note:以下设定方法都能在实现类中重新设定，这儿就改掉，不要用啦
		// now style everything!
		//TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.JazzyViewPager); //通过反设XML文件中的配置获取相关数据，现在改为全部直接设定
		int effect = 0;  //ta.getInt(R.styleable.JazzyViewPager_style, 0)   standard = 0  //默认为standard
		String[] transitions = Util.effects;
		setPageMargin(0);//两个页面之间的间距,默认设为0
		setTransitionEffect(TransitionEffect.valueOf(transitions[effect]));
		setFadeEnabled(true);  //ta.getBoolean(R.styleable.JazzyViewPager_fadeEnabled, false)  有淡入淡出效果
		setOutlineEnabled(false);	//ta.getBoolean(R.styleable.JazzyViewPager_outlineEnabled, false)	有边框
		setOutlineColor(Color.WHITE);	//ta.getColor(R.styleable.JazzyViewPager_outlineColor, Color.WHITE)	边框颜色
		switch (mEffect) {
			case Stack:
			case ZoomOut:
				setFadeEnabled(true);
